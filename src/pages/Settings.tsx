...
function BackupSection() {
  ...
  const handleSaveClientId = () => {
    ...
    setHasClientId(true);
    showStatus('success', 'Client ID saved');
  };
      ...
      <div>
        ...
        {!hasClientId ? (
          ...
          <div className="space-y-3">
            <p className="text-sm text-theme-secondary">
              To enable Google Drive backup, you need a Google Cloud Client ID.
            </p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded bg-hover border border-theme text-theme text-sm"
              />
              <button
                onClick={handleSaveClientId}
                disabled={!clientId.trim()}
                className="px-4 py-2 rounded font-semibold bg-teal text-white text-sm disabled:opacity-50"
              >
                Save
              </button>
            </div>
            ...
            >
              {showSetupInstructions ? 'Hide instructions' : 'How to get a Client ID'}
            </button>
            {showSetupInstructions && (
              <div className="text-xs text-theme-secondary bg-hover rounded p-3 space-y-1">
                <p>1. Go to <a href="https://console.cloud.google.com" target="_blank" rel="noreferrer" className="text-teal underline">console.cloud.google.com</a></p>
                <p>2. Create a new project (or select existing)</p>
                <p>3. Enable the <strong>Google Drive API</strong></p>
                <p>4. Go to <strong>Credentials → Create Credentials → OAuth 2.0 Client ID</strong></p>
                <p>5. Application type: <strong>Web application</strong></p>
                <p>6. Add your app URL to <strong>Authorized JavaScript origins</strong></p>
                <p>7. Copy the <strong>Client ID</strong> and paste above</p>
              </div>
            )}
          </div>
        ) : !connected ? (
          /* Has client ID but not connected */
          <div className="space-y-3">
            <button
            ...
            <button
              ...
              className="text-xs text-theme-secondary underline"
            >
              Remove Client ID
            </button>
          </div>
